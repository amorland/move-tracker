...
export default function HomeSubnav() {
  ...
  const current = HOME_NAV.find(item => item.href === pathname)?.label ?? 'Planning';
    ...
    <div style={{ marginBottom: 24, padding: '14px 16px', border: '1px solid var(--color-border)', borderRadius: 14, background: 'var(--color-surface)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--color-secondary)' }}>Rooms, layout, projects, and home documents.</div>
        </div>
      </div>
