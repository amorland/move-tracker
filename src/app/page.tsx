...
export default function Dashboard() {
  ...
  const getPercent = (count: number) => totalItems > 0 ? Math.round((count / totalItems) * 100) : 0;

  const inventoryMetrics = [
    { label: 'To Bring', count: bringItems.length, percent: getPercent(bringItems.length), color: 'var(--accent)' },
    { label: 'To Sell', count: sellItems, percent: getPercent(sellItems), color: '#10b981' },
    { label: 'To Donate', count: donateItems, percent: getPercent(donateItems), color: '#f59e0b' },
    { label: 'To Trash', count: trashItems, percent: getPercent(trashItems), color: '#ef4444' }
  ];
  ...
  const anchorDates = [
    ...
    { label: 'U-Pack Final Pickup', date: settings.upackFinalPickupDate, confirmed: settings.isUpackFinalPickupConfirmed }
  ];
  ...
  return (
    <div style={{ maxWidth: '100%', padding: '0 20px', paddingBottom: '40px' }}>
        ...
        <div className="card" style={{ marginBottom: 0, padding: 0, overflow: 'hidden', border: 'none', boxShadow: 'var(--shadow-md)', gridColumn: '1 / -1' }}>
          <div style={{ padding: '20px 28px', borderBottom: '1px solid var(--border)', backgroundColor: '#fafbfc', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '10px', fontWeight: 700, color: 'var(--success)' }}>
                <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--success)' }}></div> CONFIRMED
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '10px', fontWeight: 700, color: '#94a3b8' }}>
                <div style={{ width: '8px', height: '8px', borderRadius: '50%', border: '1px solid #94a3b8' }}></div> ESTIMATED
              </div>
            </div>
          </div>
          ...
          <div style={{ padding: '32px 28px' }}>
              ...
              {inventoryMetrics.map(metric => (
                <div key={metric.label}>
                  <div className="flex justify-between items-end mb-2">
                    <div>
                      <div style={{ fontSize: '13px', fontWeight: 800, color: 'var(--foreground)' }}>{metric.label}</div>
                      <div style={{ fontSize: '11px', color: 'var(--text-secondary)', fontWeight: 600 }}>{metric.count} Items</div>
                    </div>
                    <div style={{ fontSize: '18px', fontWeight: 800, color: metric.color }}>{metric.percent}%</div>
                  </div>
                  <div style={{ height: '8px', background: '#f1f5f9', borderRadius: '4px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${metric.percent}%`, background: metric.color, borderRadius: '4px', transition: 'width 0.6s ease' }}></div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #f1f5f9' }}>
               <div className="flex justify-between items-center">
                 <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--foreground)' }}>Packing Progress</div>
                 <div style={{ fontSize: '13px', fontWeight: 800, color: 'var(--accent)' }}>{packingProgress}%</div>
               </div>
               <div style={{ height: '4px', background: '#f1f5f9', borderRadius: '2px', overflow: 'hidden', marginTop: '12px' }}>
                  <div style={{ height: '100%', width: `${packingProgress}%`, background: 'var(--accent)', transition: 'width 1s ease' }}></div>
               </div>
               <p style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '8px' }}>{packedItems} of {bringItems.length} items to bring are packed.</p>
            </div>
          </div>
        </div>
